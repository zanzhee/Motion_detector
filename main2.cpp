...
const unsigned long motionDisplayDuration = 5000; // Durasi tampilan gerakan
...
void sirene() {
  for (int i = 0; i < 16; i++) { // Perulangan 8 kali untuk 8 kedipan
...
void loop() {
  ...
  if (motionDetected) {
    ...
    lastMotionTime = millis(); // Catat waktu gerakan terdeteksi
  ...
  // Jika tidak ada gerakan dan sudah lebih dari durasi tampilan
  if (millis() - lastMotionTime < motionDisplayDuration) {
    lcd.setCursor(4, 0);
